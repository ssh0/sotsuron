...
class Meeting:
    ...
    def __init__(self, K=20, N=6, S=10, a=2, p=0.5, draw=True):
        self.K = K
        self.N = N
        self.S = S
        self.a = a
    ...
    def init(self):
        ...
        if self.draw:
            ...
            self.ax.text(
                self.subject[0], self.subject[1], '0: %d' % self.N, fontsize=5)
    ...
    def callback(self):
        if self.draw:
            ...
            l1 = self.ax.plot([ix, jx], [iy, jy], color='black')
            self.ax.text(jx, jy, '%d: %d' %
                         (self.k, self.minutes[-1][1]), fontsize=20)
        else:
            pass
    ...
    def after(self):
        if self.draw:
            plugins.connect(
                self.fig, plugins.InteractiveLegendPlugin(self.s1, self.labels, ax=self.ax))
            mpld3.enable_notebook()
        else:
            print meeting.minutes

if __name__ == '__main__':
    meeting = Meeting(K=30, N=4, S=10, a=2, p=0.2)
    meeting.progress()
